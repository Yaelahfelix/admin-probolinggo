...
export default function CetakPengaduan() {
  ...
  const id = decrypt(decodeURIComponent(paramsIdValue))
	const { data: UserData, isLoading: UserLoading, isError: UserError, mutate: UserMutate } = useFetch(`/api/laporan/adu-satu/${id}`)
      ...
      <div>
        ...
        /> */}
          <div className='my-2 w-[210mm] flex justify-end'>
            <Button className='mx-[20px] ' onClick={() => reactToPrintFn()}>Print Bukti Aduan</Button>
          </div>
      </div>
      <div ref={contentRef} className={`${styles.basereport} overflow-auto border-t-0`}>
          ...
          <div className="col-span-2 border">
            <div className="grid grid-cols-2 grid-rows-8 gap-0">
              ...
              <div className='flex flex-row row-start-2 w-full justify-start items-center gap-2 py-0 px-2'>
                ...
                <p>:</p>
                <p className={`${UserData.data.status.warna} p-1 rounded-md flex-grow text-center`}>{UserData.data.status.value}</p>
              </div>
          ...
          <div className="col-start-3 border" >
            <div className="grid grid-cols-1 grid-rows-1 gap-0">
              <div className='flex flex-row w-full justify-center'>
                <h1 className='p-2 underline font-bold'>SCAN FOR TRACKING ADUAN</h1>
              </div>
              <div className='flex flex-row w-full justify-center'>
                <Canvas
        ...
        <p className='text-xs italic ml-3'>Bukti ini di buat oleh petugas : <span className='font-semibold'>{UserData.data.user_buat}</span>, dan dibuat secara elektronik, tidak dibuahi tanda tangan</p>
