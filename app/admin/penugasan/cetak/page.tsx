...
export default function CetakPenugasan() {
  ...
  const id = decrypt(decodeURIComponent(paramsIdValue))
	const { data: UserData, isLoading: UserLoading, isError: UserError, mutate: UserMutate } = useFetch(`/api/laporan/spk-satu/${id}`)
	const { data: ttdLap, isLoading: ttdLoading, isError: ttdError, mutate: ttdMutate } = useFetch('/api/laporan/get-ttd-lap', {kode : "lspk"})
      ...
      <div>
        ...
        /> */}
          <div className='my-2 w-[210mm] flex justify-end'>
            <Button className='mx-[20px] ' onClick={() => reactToPrintFn()}>Print Bukti Penugasan</Button>
          </div>
      </div>
