...
import { Check, CircleAlert, Pencil, Printer, Settings, Trash2 } from "lucide-react"
...
import { deleteData } from "@/lib/actions/penugasanAction"
  ...
  return (
    <div className="text-center flex flex-row m-auto justify-center place-content-center">
      ...
      <TooltipTrigger asChild>
        <Link href={`/admin/penugasan/cetak?id=${encodeURIComponent(encrypt(id))}`  }>
          <Printer size={18} className="cursor-pointer mt-2 mr-1"/>
        </Link>
      </TooltipTrigger>
      <TooltipContent>
          <p>Print Bukti Penugasan</p>
      </TooltipContent>
      </Tooltip>
      </TooltipProvider>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem asChild      disabled={disable}>
            <Link href={`/admin/penugasan/edit?id=${encodeURIComponent(encrypt(id))}`}>
