...
export async function DELETE(
  ...
  try {
    ...
    const [dataCheck] = await db.query<RowDataPacket[]>(
      "select id,nama from jenis_aduan where id=? order by nama asc",
      [id]
    );
    ...
    const [rows] = await db.execute<RowDataPacket[]>(
      "DELETE from jenis_aduan where id=?",
      [id]
    );
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Succes Delete Jenis Aduan",
        data: result,
      },
      { status: 200 }
    );
...
export async function PUT(
  ...
  try {
    ...
    const [rows] = await db.execute<RowDataPacket[]>(
      "UPDATE jenis_aduan set nama=?,is_active=? where id=?",
      [nama, aktif, id]
    );
...
export async function GET(
  ...
  try {
    ...
    const id = param.id;
    const [dataCheck] = await db.query<RowDataPacket[]>(
      "select id,nama,is_active from jenis_aduan where id=? order by nama asc",
      [id]
    );
